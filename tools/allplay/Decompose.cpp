...
Error decompose(StringRef BCFile, StringRef OutDir) {
  ...
  if (!M)
    return make_error<StringError>("Unable to open IR file " + BCFile, errc::invalid_argument);
  ...
  for (auto &MPart : Parts) {
    ...
    tool_output_file Out(OutName, EC,
        sys::fs::F_None);
    if (EC)
      return errorCodeToError(EC);
...
CommandRegistration Unused(&Decompose, [](ResourcePaths &RP LLVM_ATTRIBUTE_UNUSED) -> Error {
